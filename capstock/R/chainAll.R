...
#' @export
chainAll <- function(series, pairs = NULL, lastCompleteYear, parallelise = TRUE, benchType = 4) {

  # --- Input Checks -----------------------------------------------------------
  # .data must contain "data" list-column
  stopifnot("data" %in% colnames(series))
  # Get the column names from the embedded data list-column
  dataCols <- colnames(series$data[[1]])
  ...
  # --- Chain ------------------------------------------------------------------
  # .data <- unnest(.data)
  x <- dplyr::mutate(series, chained =
                       foreach(i = seq_len(nrow(series)),
                               .packages = c("dplyr", "capstock"),
                               .errorhandling = "pass") %doOrDoPar%
                       dplyr::bind_cols(
                         foreach(p = seq_len(nrow(pairs))) %do%
                           chainRename(series = as.data.frame(series[[i, "data"]]),
...
chainRename <- function(series, lastCompleteYear, cypColumn, pypColumn, chainType, benchType) {
  x <- robustChain(series, lastCompleteYear, cypColumn, pypColumn, chainType, benchType = 4)
...
#' @importFrom magrittr "%>%"
#' @export
robustChain <- function(series, lastCompleteYear, cypColumn = "CYP", pypColumn = "PYP", chainType = c("Stock", "Flow"), benchType = 4) {
  # If the entire series is zero (often the case with adjustment values) just
  # return CVM as all zeros
  if (all(!is.na(series[cypColumn]) & series[cypColumn] == 0)) {
    series["CVM"] <- 0
    return(series)
  }
  ...
  year1 <- seq(1, 4)
  if (all(is.na(series[year1, pypColumn])) | all(series[year1, pypColumn] == 0)) {
    leadingRows <- series[year1, ]
    partialResult <- chain(series = series[-year1, ],
                           lastCompleteYear = lastCompleteYear,
...
chainAll_old <- function(.data, pairs = NULL, lastCompleteYear, parallelise = TRUE, benchType = 4) {
  ...
  x <- dplyr::mutate(.data, chained =
                       ...
                       dplyr::bind_cols(
                         foreach(p = seq_len(nrow(pairs))) %do%
                           chainRename_old(.data = .data[[i, "data"]],
                                           lastCompleteYear = lastCompleteYear,
                                           chainType = pairs$ChainType[p],
                                           cypColumn = pairs$CYP[p],
                                           pypColumn = pairs$PYP[p], benchType = benchType)))
...
# Helper function to rename "CVM" column from chain results based on the supplied cypColumn
chainRename_old <- function(.data, lastCompleteYear, cypColumn, pypColumn, chainType, benchType) {
  x <- robustChain_old(.data, lastCompleteYear, cypColumn, pypColumn, chainType, benchType = 4)
  x <- dplyr::select(x, CVM)
  # Rename cols to be cypColumn root + CVM, e.g. CapitalServicesCYP/PYP => CapitalServicesCVM
  # If the cypColumn doesn't have CYP at the end, just use the cypColumn
  # x <- .data
...
#' @importFrom magrittr "%>%"
#' @export
robustChain_old <- function(.data, lastCompleteYear, cypColumn = "CYP", pypColumn = "PYP", chainType = c("Stock", "Flow"), benchType = 4) {
  # If the entire series is zero (often the case with adjustment values) just
  # return CVM as all zeros
  ...
  if (all(is.na(.data[year1, pypColumn])) | all(.data[year1, pypColumn] == 0)) {
    ...
    partialResult <- chain_old(.data = .data[-year1, ],
                               lastCompleteYear = lastCompleteYear,
                               cypColumn = cypColumn, pypColumn = pypColumn,
                               chainType = chainType, benchType = benchType)
    result <- dplyr::bind_rows(leadingRows, partialResult)
  } else {
    result <- chain_old(.data = .data[, ],
                        lastCompleteYear = lastCompleteYear,
                        cypColumn = cypColumn, pypColumn = pypColumn,
                        chainType = chainType, benchType = benchType)
  }
