...
#' @export
unchainAll <- function(df, pairs, refYear, parallelise = TRUE) {
  ######################################################################################################
  # .data <- out
  ...
  stopifnot("data" %in% colnames(df))
  # Get the column names from the embedded data list-column
  dataCols <- colnames(df$data[[1]])
  ...
  #     unchain
  x <- dplyr::mutate(df, unchained =
                       foreach(i = seq_len(nrow(df)),
                               .packages = c("dplyr", "capstock"),
                               .errorhandling = "pass") %doOrDoPar%
                       dplyr::bind_cols(
                               purrr::map2(pairs$CP, pairs$flow_stock,
                                       ~unchainRename(series = as.data.frame(df[[i, "data"]]),
...
unchainRename <- function(series, cpColumn, flow_stock, refYear) {
  x <- unchain(series, cpColumn, flow_stock,refYear)
  ...
  names(x)[names(x) == 'CYP'] <- paste0(varNameRoot, "CYP")
  names(x)[names(x) == 'PYP'] <- paste0(varNameRoot, "PYP")
  return(x)
}
