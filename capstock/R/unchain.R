...
#'
#' @param values A data.frame with the current price, chained volume measures and
...
#' @export
unchain <- function(values, cpColumn = "CP", flow_stock = NULL, refYear = refYear){
  ...
  result <- dplyr::mutate(result, CP = !!sym(cpColumn), KP = !!sym(cvmColumn))
  ...
  if (flow_stock == "Flow"){
    ...
    result <- dplyr::group_by(result, Year)
