...
postProcess <- function(timeSeries, pimResults, referenceYear, discountRate){
  result <- timeSeries %>%
    ...
    dplyr::mutate(GrossStockCP = GrossStock*PriceIndex,
                  NetStockCP = NetStock*PriceIndex,
                  ProductiveStockCP = ProductiveStock*PriceIndex,
                  TotalChangesInVolumeCP = TotalChangesInVolumeCVM*PriceIndex,
                  ConsumptionOfFixedCapitalCP = ConsumptionOfFixedCapitalCVM*PriceIndex,
                  NetFixedCapitalFormationCP = NetFixedCapitalFormationCVM*PriceIndex) %>%
